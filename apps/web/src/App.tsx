...
const tabs:{id:Tab;labelKey:string;icon:typeof Home}[]=[{id:'home',labelKey:'nav.today',icon:Home},{id:'log',labelKey:'nav.food',icon:Apple},{id:'progress',labelKey:'nav.progress',icon:BarChart3},{id:'rewards',labelKey:'nav.rewards',icon:Sparkles},{id:'profile',labelKey:'nav.profile',icon:CircleUserRound}];
export default function App(){const {t}=useTranslation();const {data,isLoading}=useQuery({queryKey:['dashboard'],queryFn:api.dashboard});const tab=useAppStore(state=>state.tab);const setTab=useAppStore(state=>state.setTab);const setAddMealOpen=useAppStore(state=>state.setAddMealOpen);const toast=useAppStore(state=>state.toast);const theme=useAppStore(state=>state.theme);
...
<button title={t('food.title')} aria-label={t('food.title')} onClick={()=>{haptic('medium');setAddMealOpen(true)}} className="fixed bottom-[84px] left-1/2 z-50 flex h-14 w-14 -translate-x-1/2 items-center justify-center rounded-full bg-blue text-white shadow-float"><Plus size={26} strokeWidth={2.7}/></button>
<nav className="bottom-nav"><div className="grid grid-cols-5 gap-1">{tabs.map((item,index)=>{const Icon=item.icon;const active=item.id===tab;return <button key={item.id} onClick={()=>{haptic();setTab(item.id)}} className={cn('flex h-14 min-w-0 flex-col items-center justify-center gap-1 rounded-lg text-[10px] font-semibold transition-colors',index===2?'mr-7':'',index===3?'ml-7':'',active?'text-blue':'text-[#8b96a3]')}><Icon size={21} strokeWidth={active?2.7:2}/><span>{t(item.labelKey)}</span></button>})}</div></nav>
