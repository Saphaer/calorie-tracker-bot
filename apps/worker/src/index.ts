...
async function runReminders(env:Bindings){if(!env.TELEGRAM_BOT_TOKEN)return; const hour=new Date().getUTCHours(); const rows=await env.DB.prepare(`SELECT u.telegram_id,u.first_name,s.daily_reminder,s.water_reminder,s.streak_warning,s.reminder_hour,coalesce(dp.water_ml,0) water_ml,u.water_target_ml,st.last_active_date FROM users u JOIN settings s ON s.user_id=u.id JOIN streaks st ON st.user_id=u.id LEFT JOIN daily_progress dp ON dp.user_id=u.id AND dp.date=? WHERE u.onboarding_complete=1`).bind(today()).all<Record<string,unknown>>(); for(const row of rows.results){let text=''; if(Boolean(row.daily_reminder)&&Number(row.reminder_hour)===hour)text=`Good morning, ${row.first_name}. Your Nourish plan is ready.`; else if(Boolean(row.water_reminder)&&hour===16&&Number(row.water_ml)<Number(row.water_target_ml)*0.6)text=`Hydration check: you are at ${row.water_ml} ml today. A glass of water keeps your goal moving.`; else if(Boolean(row.streak_warning)&&hour===20&&row.last_active_date!==today())text='Your daily streak is still waiting. Log one meal or glass of water to keep it alive.'; if(text)await sendTelegram(env.TELEGRAM_BOT_TOKEN,String(row.telegram_id),text,env.APP_ORIGIN);}}
